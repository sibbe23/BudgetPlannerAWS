...
async function addexpense(e){
 ...
 try{
    ...
    const response =await axios.post('http://52.90.35.31:22/expense/add-expense',expensedetails,{headers:{"Authorization":token}});
...
window.addEventListener('DOMContentLoaded',async ()=>{
    ...
    const response =await axios.get(`http://52.90.35.31:22/expense/get-expenses?page=${page}&limit=${limit}`,{headers:{"Authorization":token}});
    ...
    try{
       ...
       const response= await axios.delete(`http://52.90.35.31:22/expense/delete-expense/${id}`,{headers:{"Authorization":token}});
        total=response.data.totalExpense;
        totalexpense.innerHTML=total;
   ...
   async function premiumuser(e){
    try{
        const token=localStorage.getItem('token');
        console.log(token);
        const response =await axios.get(`http://52.90.35.31:22/purchase/premiummembership`,{headers:{'Authorization':token}});
        console.log(response);
        var options={
            "key":response.data.key_id,
            "order_id":response.data.order.id,
            "handler":async function(response){
               const result= await axios.post('http://52.90.35.31:22/purchase/updatetransactionstatus',{
   ...
   async function showleaderboard(){
    ...
    const token=localStorage.getItem('token');
    const sortedarray= await axios.get(`http://52.90.35.31:22/premium/leaderboard`,{headers:{'Authorization':token}});
   ...
   async function downloadexpenses(e){
    ...
    try{
        const token=localStorage.getItem('token');
        const response  =  await axios.get(`http://52.90.35.31:22/expense/download`,{headers:{'Authorization':token}});
   ...
   async function downloadedfiles(e){
            ...
            try{
                e.preventDefault();
               const token= localStorage.getItem('token');
            const response= await axios.get(`http://52.90.35.31:22/expense/downloadedfiles`,{headers:{'Authorization':token}});
